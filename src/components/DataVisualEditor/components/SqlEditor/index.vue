...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initHints() {
        let sqlArr = sqlKeys.split(" ");

        if (!this.tableNameKeys)
          this.tableNameKeys = []
        if (!this.fieldNameKeys)
          this.fieldNameKeys = []
        this.hints = Array.from(new Set([...this.hints, ...sqlArr, ...this.tableNameKeys, ...this.fieldNameKeys])).sort();
      },
      init() {
        let that = this;
        this.dispose();
        ...
        this.monacoInstance = monaco.editor.create(this.$refs["container"], {
          value: this.value,
          autoIndex: true,
          theme: "myTheme",  //  vs, hc-black, vs-dark
          fontSize: 18,
