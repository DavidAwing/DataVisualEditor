  ...
  <div class="container">
    ...
    <div class="style-list">
      <el-form label-position="left">
        <el-popover placement="left" title="" width="300" trigger="hover" :open-delay="1500"
          ...
          <el-form-item label="设置项" slot="reference">
    ...
    <div class="style-css">
      <div class="style-item">
        <el-form label-position="left">
          <el-tag :key="generateStyleId(style.styleId)" v-for="style in addedStyleTags" closable
            :disable-transitions="false" @close="handleStyleTagsClose(style)" style="font-size: 12px" v-if="false">
            {{ style.styleName }}
          </el-tag>
          ...
          <el-collapse v-model="cssActiveCollapses">
            <el-collapse-item title="已应用样式" name="1" v-if="false">
              <el-tag v-for="(style, styleIndex) in addedStyleTags" :key="generateStyleId(style.styleId) + styleIndex"
                closable :disable-transitions="false" @close="handleStyleTagsClose(style)" @click="switchToStyle(style)"
                class="applied-style">
                {{ style.styleName }}
              </el-tag>
            </el-collapse-item>
            ...
            <el-collapse-item title="设置" name="2">
              <div style="display: flex; flex-flow: column nowrap">
                <el-form-item v-for="({ label, variable, type, options }, index) in curStyle.attrList"
                  :key="index + '-' + type" :label="label || variable">
                  <span slot="label">
                    <span>
                      <span>{{ label }}</span>

                      <template v-for="(icon, iconIndex) in options.iconList">
                        <el-button class="array-icon" :key="iconIndex" type="primary" :icon="icon.icon" size="mini"
                          style="transform: scale(0.65)" v-if="icon.show !== false" circle @click="
                            () => {
                  ...
                  <div v-if="type === 'color-picker'">
                    <div v-if="options.gradientType === 'linear'"
                      ...
                      <div style="flex: 1; width: 100%; display: flex; flex-flow: row nowrap; justify-content: center">
                        ...
                        <span style="margin-left: 10px">
                          <el-input-number v-model.number="curStyle.attrList[index].options.value.x" type="number"
                            :min="0" :max="1" :step="0.1" /></span>
                      </div>
                      ...
                      <div style="
                        ...
                        <span style="margin-left: 10px">
                          <el-input-number v-model.number="curStyle.attrList[index].options.value.y" type="number"
                            :min="0" :max="1" :step="0.1" /></span>
                      </div>
                      ...
                      <div style="
                        ...
                        <span style="margin-left: 10px">
                          <el-input-number v-model.number="curStyle.attrList[index].options.value.x2" type="number"
                            :min="0" :max="1" :step="0.1" /></span>
                      </div>
                      ...
                      <div style="
                        ...
                        <span style="margin-left: 10px">
                          <el-input-number v-model.number="curStyle.attrList[index].options.value.y2" type="number"
                            :min="0" :max="1" :step="0.1" /></span>
                      </div>
                      ...
                      <div style="
                        ...
                        <div style="display: flex; justify-content: center">
                          <span style="margin-left: 10px">
                            <el-input-number v-model.number="item.offset" type="number" :min="0" :max="1"
                              :step="0.1" /></span>
                    ...
                    <div v-else-if="options.gradientType === 'radial'"
                      style="display: flex; width: 100%; position: relative; flex-flow: column nowrap">
                      辐射状的渐变
                    </div>
                  ...
                  <div v-else-if="type === 'text'" style="display: flex; width: 100%; position: relative"
                    class="form-item-input">
                    <el-input v-model="curStyle.attrList[index].value" @change="
                        (val, old) => {
                  ...
                  <div v-else-if="type === 'number'" style="display: flex; width: 100%; position: relative">
                    ...
                    <el-select v-if="curStyle.attrList[index].options.unit"
                      v-model="curStyle.attrList[index].options.unit" style="width: 100px; margin-left: 6px" @change="
                        (val, old) => {
                          onStyleAttrEvent([...options['onChange']], {
                            attrIndex: index,
                            attr: curStyle.attrList[index],
                  ...
                  <div v-else-if="type === 'integer'" v-show="options.show === false ? false : true"
                    style="display: flex; width: 100%; position: relative">
                    <!--  oninput="value=value.replace(/[^0-9]/g,'')"     :max="options.max !== undefined ? options.max : 99999"-->
                    <el-input-number v-model.number="curStyle.attrList[index].value"
                      :min="options.min !== undefined ? options.min : -999999999"
                      :max="options.max !== undefined ? options.max : 999999999"
                      :step="options.step !== undefined ? options.step : 1" type="number" @change="
                        (val, old) => {
                          onStyleAttrEvent([...options['onChange']], {
                            attrIndex: index,
                            attr: curStyle.attrList[index],
                  ...
                  <div v-else-if="type == 'select'">
                    <el-select v-model="curStyle.attrList[index].value" placeholder="" @change="
                        (val, old) => {
                          onStyleAttrEvent([...options['onChange']], {
                            attrIndex: index,
                            attr: curStyle.attrList[index],
                      ...
                      ">
                      <el-option v-for="item in options.options" :key="item.value" :label="item.label"
                        :value="item.value">
                      </el-option>
                    </el-select>
                  </div>
                  ...
                  <div v-else-if="type === 'string[]'">
                    <div v-for="(item, itemIndex) in curStyle.attrList[index].value">
                      <el-input v-model="curStyle.attrList[index].value[itemIndex]"
                        :type="type === 'string[]' ? 'text' : 'number'" @focus="
                          () => {
                  ...
                  <div v-else-if="type === 'number[]' || type === 'integer[]'">
                    <div v-for="(item, itemIndex) in curStyle.attrList[index].value"
                      style="display: flex; width: 100%; position: relative">
                      <el-input-number v-model.number="curStyle.attrList[index].value[itemIndex]" :step="1"
                        type="number" />
                    </div>
                  </div>

                  <div v-else-if="type === 'number-color-array'">
                    <div v-for="(item, itemIndex) in curStyle.attrList[index].value" style="
                        flex: 1;
                        width: 100%;
                        display: flex;
                        flex-flow: row nowrap;
                      ...
                      <div style="display: flex; justify-content: center">
                        ...
                        <span style="margin-left: 10px; flex: 1">
                          <el-input-number v-model.number="item[0]" type="number" :min="options.min" :max="options.max"
                            :step="options.step" /></span>
                        <span style="margin-left: 10px">
