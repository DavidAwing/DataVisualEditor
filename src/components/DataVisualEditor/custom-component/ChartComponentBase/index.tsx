...
export default class ChartsComponentBase extends ComponentBase {
  ...
  public static ChartTemplateList = [] as any[]



  public static init = (function () {
    // 静态方法的实际实现
    let hasExecuted = false; // 用于跟踪方法是否已执行
    ...
    return function () {
      if (!hasExecuted) {
        ...
        axios.get(`/BI-API/Component/GetUserCustomizedComponentList?type=.vct`).then(({ data }) => {
          for (const item of data.data) {
            ChartsComponentBase.ChartTemplateList.push(JSON.parse(item));
          }
        });


        hasExecuted = true; // 将状态标记为已执行
      } else {
        console.log('静态方法已执行，不再执行');
      }
    };
  })();
  ...
  public mounted() {
    ...
    const fn = () => {
      ...
      eventBus.$on('SetOption', (name: string, newOption: any) => {
        ...
        if (name !== this.element.data.name) return
        function correctiveData(obj: any) {
          for (let prop in obj) {
            if (obj[prop] === null) {
              delete obj[prop];
