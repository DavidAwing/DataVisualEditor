...
export function changeStyleWithScale(value, attributeName, unit) {

  value = multiply(value, divide(parseInt(store.state.canvasData.scale), 100))

  if (unit === '%' && attributeName != null && attributeName.toLowerCase().indexOf("width") !== -1) {
    value = document.body.clientWidth * (value / 100)
  } else if (unit === '%' && attributeName != null && attributeName.toLowerCase().indexOf("height") !== -1) {
    // value = window.screen.height * (value / 100)
    value = document.body.clientHeight * (value / 100)
  }
  return value
}
