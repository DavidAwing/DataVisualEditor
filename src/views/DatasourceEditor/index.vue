<template>
  <div class="container">
    <div>
      ...
      <div>
        ...
        <el-select v-model="canvasDataSource.componentName" placeholder="" :multiple="false" clearable>
          <el-option v-for="item in ComponentList" :key="item.data.name" :label="item.data.name"
      ...
      <div>
        ...
        <div style="display: flex; align-items: center; width: 26px; justify-content: center"
          v-if="canvasDataSource.dataSourceType === 'database'" @click="switchPattern">
      ...
      <div class="canvas-data-source-list">
        <div>
          <span>已添加的数据源</span>
          <el-button style="margin-left: 6px; transform: scale(0.6)" @click="addCanvasDataSource" icon="el-icon-plus"
            type="primary" size="mini" circle></el-button>
          <el-button style="transform: scale(0.6) translateX(-13px)" @click="deleteCanvasDataSource"
            icon="el-icon-minus" type="primary" size="mini" circle></el-button>
        </div>
        ...
        <div>
          ...
          <div v-for="(item, index) in canvasDataSourceList" @click="selectCanvasDataSource(item)"
            :class="getDataSourceClassName(item)" :key="index">
            <div>{{ item.name }}</div>
    ...
    <div v-if="canvasDataSource.dataSourceType === 'database' && databasePattern === 'QueryBuilder'">
      <iframe id="QueryBuilder" key="QueryBuilder" @load="onQueryBuilderLoad"
        src="http://127.0.0.1:5502/sql-build/index.html" height="100%" width="100%" frameborder="no" border="0"
        marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" seamless allowfullscreen="true"
    ...
    <el-dialog title="组件预览" :visible.sync="componentPreviewDialogVisible" class="component-preview">
      <div>
        ...
        :element="Component"
      > -->
        <component :is="Component.component" :id="'component' + Component.id" class="component"
          :style="getComponentStyle(Component)" :prop-value="Component.propValue" :element="Component" />
        <!-- </Shape> -->
      </div>
    </el-dialog>
    ...
    methods: {
      addDatabase(type, event) {
        if (type === 'show') {
          ...
          this.addDatabaseDialogTitle = '删除或更新数据库?';
          this.addDatabaseDialogVisible = true;
        } else if (type === 'remove') {
          this.addDatabaseDialogVisible = false;
