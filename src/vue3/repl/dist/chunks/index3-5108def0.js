  ...
  Object.defineProperty(Rule.prototype, 'debugName', {
    get: function () {
      return (
        ...
        this.id +
        ' @ ' +
        path.basename(this.$location.filename) +
        ':' +
        this.$location.line
      );
    },
...
var ScopeMetadataProvider = /** @class */ (function () {
  ...
  ScopeMetadataProvider.prototype.onDidChangeTheme = function () {
    ...
    this._defaultMetaData = new ScopeMetadata('', this._initialLanguage, 0 /* Other */, [
...
var Grammar = /** @class */ (function () {
  ...
  Grammar.prototype._tokenize = function (lineText, prevState, emitBinaryTokens) {
    if (this._rootId === -1) {
      this._rootId = rule_1.RuleFactory.getCompiledRuleId(
    ...
    if (!prevState || prevState === StackElement.NULL) {
      ...
      var defaultMetadata = StackElementMetadata.set(
        0,
...
function handleCaptures(grammar, lineText, isFirstLine, stack, lineTokens, captures, captureIndices) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (captureRule.retokenizeCapturedWithRuleId) {
      ...
      _tokenizeString(
        grammar,
        rule_1.createOnigString(rule_1.getString(lineText).substring(0, captureIndex.end)),
...
function _checkWhileConditions(grammar, lineText, isFirstLine, linePos, stack, lineTokens) {
  ...
  for (var whileRule = whileRules.pop(); whileRule; whileRule = whileRules.pop()) {
    var ruleScanner = whileRule.rule.compileWhile(
      grammar,
      ...
      if (_rule instanceof rule_1.BeginEndRule) {
        ...
        if (pushedRule.endHasBackReferences) {
          stack = stack.setEndRule(
            pushedRule.getEndWithResolvedBackReferences(rule_1.getString(lineText), captureIndices)
          );
        }
        ...
        if (pushedRule.whileHasBackReferences) {
          stack = stack.setEndRule(
            pushedRule.getWhileWithResolvedBackReferences(rule_1.getString(lineText), captureIndices)
          );
        }
        if (!hasAdvanced && beforePush.hasSameRuleAs(stack)) {
          // Grammar pushed the same rule without advancing
...
var SyncRegistry = /** @class */ (function () {
  ...
  SyncRegistry.prototype.grammarForScopeName = function (scopeName, initialLanguage, embeddedLanguages, tokenTypes) {
    if (!this._grammars[scopeName]) {
      ...
      this._grammars[scopeName] = grammar_1$1.createGrammar(
        rawGrammar,
        initialLanguage,
        embeddedLanguages,
        tokenTypes,
...
function resolveParsedThemeRules(parsedThemeRules) {
  ...
  var defaults = new ThemeTrieElementRule(
    0,
    null,
    defaultFontStyle,
...
var Registry = /** @class */ (function () {
   ...
   */
  Registry.prototype.loadGrammarWithEmbeddedLanguages = function (
    initialScopeName,
    initialLanguage,
    embeddedLanguages
  ) {
                  ...
                  switch (_a.label) {
                    ...
                    case 1:
                      ...
                      injections =
                        ...
                        this._locator.getInjections(initialScopeName);
        ...
        tokenize: (line, state) => {
          ...
          return {
            ...
            tokens: res.tokens.map(token => ({
              ...
              scopes: editor
                ? (0, tm_to_monaco_token_1.TMToMonacoToken)(editor, token.scopes)
